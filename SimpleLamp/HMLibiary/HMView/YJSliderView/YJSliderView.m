...
- (void)bindStyleButton:(UIButton *)btn redNum:(NSInteger)redNum status:(BOOL)isSelected {
    ...
    [self.titleButton setTitleColor:isSelected ?  self.themeColor ? self.themeColor : [UIColor colorWithRed:0.15 green:0.71 blue:0.96 alpha:1.00] : [UIColor grayColor] forState:UIControlStateNormal];
...
- (CGFloat)yj_calculateItemWithAtIndex:(NSInteger)index {
    NSNumber *width = [self.titleWidthCache objectForKey:[NSString stringWithFormat:@"%ld", (long)index]];
    if (width) {
        ...
        [self.titleWidthCache setObject:@(itemWidth) forKey:[NSString stringWithFormat:@"%ld", (long)index]];
...
- (UICollectionView *)titleCollectionView {
    if (_titleCollectionView == nil) {
        ...
        _titleCollectionView.backgroundColor = self.bgColor ? self.bgColor : [UIColor whiteColor];
        [self addSubview:_titleCollectionView];
        self.lineView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_titleCollectionView.frame), CGRectGetWidth(self.frame), 0.5)];
        self.lineView.backgroundColor = self.lineViewColor ? self.lineViewColor : [UIColor colorWithRed:0.93 green:0.93 blue:0.93 alpha:1.00];
...
- (UIView *)sliderLine {
    if (_sliderLine == nil) {
        ...
        _sliderLine.backgroundColor = self.sliderLineColor ? self.sliderLineColor : [UIColor colorWithRed:0.15 green:0.71 blue:0.96 alpha:1.00];
